	...
	_getTo: function (e, attrObject, toAttribute)
		{
			...
			if (attrObject.type == 'float') {
				...
				xseen.debug.logInfo("Interpolating field '" + toAttribute + "' as float.");

			} else if (attrObject.type == 'vec3') {
				interpolation = TWEEN.Interpolation.Linear;
				to = xseen.Parser.Types.vecToXYZ(to, {'x':0,'y':0,'z':0});
				xseen.debug.logInfo("Interpolating field '" + toAttribute + "' as 3-space.");

			} else if (attrObject.type == 'color') {
				interpolation = xseen.Tags.animate.Interpolator.color;
				to = new THREE.Color (xseen.Parser.Types.color(to));
				xseen.debug.logInfo("Interpolation field '" + toAttribute + "' as color.");
				...
				xseen.debug.logInfo("Interpolation field '" + toAttribute + "' as rotation.");

			} else {
				xseen.debug.logInfo("Field '" + toAttribute + "' not converted to THREE format. No animation performed.");
...
/*
			...
			if (typeof(attributes[toAttrIndex]) === 'undefined') {
				xseen.debug.logInfo("Field '" + toAttribute + "' not found in parent (" + p.localName.toLowerCase() + "). No animation performed.");
				return;
			}
	...
	'fin'	: function (e,p)
		{
			...
			var duration = 4 * 1000;	// TEMP: Convert to milliseconds
 ...
 */
			...
			var toAttribute = e._xseen.attributes.attribute;
			if (e._xseen.key.length == 0) {		// Block handels no key frames
				var target = xseen.Tags.animate._getTo (e, e._xseen.attrObject, e._xseen.attributes.attribute);
				if (target.to === null) {return; }
			
// --> TODO: Develop method when attribute value needs to be handled via setter
				var fieldTHREE, useUpdate, tween, startingValue;
				if (typeof(p._xseen.animate[toAttribute]) === 'string') { 
